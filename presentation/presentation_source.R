...
p1 <- mapview(y, zcol = "LAeq", col.regions = pal, direction=-1, layer.name="LAeq (dB)") +
  mapview(map, alpha.regions=0.4, layer.name="Madrid") +
  mapview(t_sf, layer.name="StudyArea", hide=T)
...
p2 <- mapview(v, zcol = "LAeq", col.regions = viridis::magma(10), alpha = 0.5, layer.name="LAeq")
# -------------------------
# 2. Covariates Plot -----------------------------------------------------------------------
# -------------------------
elev_c <- terra::crop(elev, vect(t_sf)) |> mask(vect(t_sf))
p3 <- mapview(elev_c, alpha = 0.5, layer.name="elevation")
ndvi <- terra::crop(ndvi, vect(t_sf)) |> mask(vect(t_sf))
p4<-mapview(ndvi, alpha = 0.5, layer.name="NDVI")
p5<-mapview(yp2, alpha = 0.1, layer.name="Prediction Points") + 
  mapview(roads_major_sf, color = "red", hide=T) +
  mapview(roads_secondary_sf, color = "blue", hide=T) +
  mapview(roads_local_sf, color = "lightblue", hide=T)
...
p6<-mapview(r_noise_mean, alpha=0.6, layer.name="predicted mean noise")+ 
...
p7<-mapview(r_noise_sd, alpha=0.6, at = c(0, 1, 2, 3, 40), layer.name="sd")
# -------------------------
...
p8<-mapview(r_excprob, alpha=0.6, layer.name="P(n>62)") + 
  mapview(y, zcol="LAeq", hide=TRUE, layer.name="Station")
...
pal <- colorNumeric("magma", c(0, 1), na.color = "transparent")
p9<-mapview(r_nonexcprob, alpha=0.6, layer.name="P(n<56)") + 
  mapview(y, zcol="LAeq", hide=TRUE, layer.name="Station") 

save(p1,p2,p3,p4,p5,p6,p7,p8,p9,
     file = "presentation/slides-data.rda")
